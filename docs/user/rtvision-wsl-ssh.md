...
| `t3 --version` works but adding still tries to download a missing version | A global executable is insufficient. Current desktops can reuse a complete service installation at the same version. The older 0.0.41 launcher requires the exact `npx --package` verification from the remote home directory. |
