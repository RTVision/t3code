...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(script, "could not install '@rtvision/t3@latest'");
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --registry 'https://npm-registry.rtvision.com/' --package '@rtvision/t3@latest'",
    );
    assert.include(
      script,
      "require_installed_t3_cli npm exec --yes --registry 'https://npm-registry.rtvision.com/' --package '@rtvision/t3@latest'",
    );
  ...
  it("shell-quotes package specs in the remote t3 runner", () => {
    ...
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --registry 'https://npm-registry.rtvision.com/' --package 't3@nightly; touch /tmp/t3-owned'",
    );
