...
/** tea writes one HTTP response header block to stderr, after any diagnostic output. */
export function parseTeaResponse(stderr: string) {
  const lines = stderr.split(/\r?\n/);
  const start = lines.findIndex((line) => /^HTTP\/\S+ \d{3}(?:\s|$)/u.test(line));
  const status = Number(/^HTTP\/\S+ (\d{3})/u.exec(lines[start] ?? "")?.[1]);
  const headers: Record<string, string> = {};
  if (start >= 0) {
    for (const line of lines.slice(start + 1)) {
      if (line === "") break;
      const match = /^([!#$%&'*+.^_`|~\w-]+):\s*(.*)$/u.exec(line);
...
export interface ForgejoApiInput extends ForgejoRepositoryInput {
  readonly resolvedTarget?: ForgejoRepository;
...
export const make = Effect.gen(function* () {
  ...
  const api = Effect.fn("ForgejoCli.api")(function* (input: ForgejoApiInput) {
    ...
    const result = yield* execute({
      ...
      maxOutputBytes: input.maxOutputBytes ?? 8 * 1024 * 1024,
    });
    // tea reports HTTP failures with exit code zero; use its response status.
    const { status, headers } = parseTeaResponse(result.stderr);
    const retryAt =
      status === 429
        ? retryAtFromHeader(headers["retry-after"], yield* Clock.currentTimeMillis)
        : undefined;
  ...
  return ForgejoCli.of({
    ...
    getAccount,
    resolveRepository,
    resolveServer: (input) => resolveTarget(input, true),
    api,
  });
});
